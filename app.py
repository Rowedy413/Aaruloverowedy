...
tasks = {}  # Dictionary to hold active tasks. Format: task_id -> {'status': 'running/complete', 'details': [...]}
...
def react_task(task_id, tokens, post_ids, reaction_type):
    ...
    for token in tokens:
        token = token.strip()
        print(f"[Task {task_id}] Using token: {token[:10]}...")  # show partial token for privacy
        for post_id in post_ids:
            ...
            time.sleep(20)  # Delay between tokens as requested
